...
import { ARTIST_ACCOUNT_PHOTO_EDIT_RESET, ARTIST_ACCOUNT_PHOTO_REMOVE_RESET } from '../../../../../../../../../redux/artist/account/photo/constants';
...
const Container = ({ open, setOpen }) => {
    ...
    const { loading: loadingRemove, success: successRemove } = useSelector(state => state.artistAccountPhotoRemove);
    ...
    useEffect(() => {
        if (successRemove) {
            dispatch({ 
                type: ARTIST_ACCOUNT_PHOTO_REMOVE_RESET,
            })
            dispatch(getArtistDetails());
        };
    }, [successRemove, ARTIST_ACCOUNT_PHOTO_REMOVE_RESET]);
    useEffect(() => {
        if (data) {
            ...
            <div className = {styles.body}>
                ...
                <form className={styles.form}>
                    ...
                    <button className = {styles.remove_btn} onClick = {removeHandler}>
                        {loadingRemove && <CircularProgress size={16} sx = {{ color: '#444' }} /> }
                        <span>حذف</span>
                    </button>
                    }
