...
export const addNumber = expressAsyncHandler(async(req, res) => {
    ...
    if (artistExists) {
        res.json({
            _id: artistExists._id,
            mobile: artistExists.mobile,
            isVerified: artistExists.isVerified,
            token: generateToken(artistExists._id),
        });
    } else {
        const artist = new Artist({
            mobile,
        });
        const createdArtist = artist.save();
        res.json({
            _id: createdArtist._id,
            mobile: createdArtist.mobile,
            isVerified: createdArtist.isVerified,
            token: generateToken(createdArtist._id),
        });
    };
